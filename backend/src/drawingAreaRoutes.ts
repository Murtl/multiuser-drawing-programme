...
const drawingAreaRouter = express.Router();

/**
 * Endpunkt zum Erstellen einer neuen Zeichenfläche.
 */
drawingAreaRouter.post(
  ...
  (req: Request, res: Response) => {
    ...
    if (userIndex !== -1) {
      users[userIndex].drawingAreas.push({
        id,
        permission: DrawingAreaPermissions.OWNER,
      });
    } else {
...
drawingAreaRouter.delete(
  ...
  (req: Request, res: Response) => {
     ...
     */
    const authorizedUserInsideDrawingAreaIndex = drawingAreas[
      drawingAreaIndex
     ...
     */
    users.forEach((user) => {
      const drawingAreaIndex = user.drawingAreas.findIndex(
        (drawingArea) => drawingArea.id === drawingAreaId
      );
      if (drawingAreaIndex !== -1) {
  ...
  (req: Request, res: Response) => {
    ...
    if (
      drawingAreas[drawingAreaIndex].authorizedUsers.find(
        (authorizedUser) => authorizedUser.userId === userIdToAdd
      )
    ) {
      return res.status(400).json({
     ...
     */
    if (permission === DrawingAreaPermissions.OWNER) {
      return res.status(400).json({
     ...
     */
    if (permission === DrawingAreaPermissions.COOWNER) {
      if (
        !drawingAreas[drawingAreaIndex].authorizedUsers.find(
          (authorizedUser) =>
            authorizedUser.userId === userId &&
        ...
        return res.status(400).json({
          message:
            'User hat nicht die Berechtigung, einen Co-Owner hinzuzufügen!',
        });
      }
    }
     ...
     */
    if (permission === DrawingAreaPermissions.MODERATOR) {
      if (
        !drawingAreas[drawingAreaIndex].authorizedUsers.find(
          (authorizedUser) =>
            authorizedUser.userId === userId &&
        ...
        return res.status(400).json({
          message:
            'User hat nicht die Berechtigung, einen Moderator hinzuzufügen!',
        });
      }
    }
...
drawingAreaRouter.post(
  ...
  (req: Request, res: Response) => {
     ...
     */
    const userAuthorized = drawingArea.authorizedUsers.find(
      (authorizedUser) => authorizedUser.userId === userId
    );
    if (!userAuthorized) {
      return res.status(400).json({
     ...
     */
    if (
      !drawingArea.authorizedUsers.find(
        (authorizedUser) =>
          authorizedUser.userId === userId &&
          (authorizedUser.permission === DrawingAreaPermissions.OWNER ||
            authorizedUser.permission === DrawingAreaPermissions.COOWNER ||
    ...
    if (
      ...
      return res.status(200).json({
        ...
        users: drawingArea.authorizedUsers.filter(
          (authorizedUser) =>
            authorizedUser.permission !== DrawingAreaPermissions.OWNER
        ),
      });
...
drawingAreaRouter.delete(
  ...
  (req: Request, res: Response) => {
     ...
     */
    const userToRemoveIndex = drawingAreas[
      drawingAreaIndex
    ].authorizedUsers.findIndex(
      (authorizedUser) =>
        authorizedUser.userId === userIdToRemove &&
        authorizedUser.permission === DrawingAreaPermissions.OWNER
    );
    ...
    if (
      ...
      if (userToRemoveIndex !== -1) {
        return res.status(400).json({
          message:
            'User hat nicht die Berechtigung, einen Co-Owner zu entfernen!',
        });
      }
    }
    ...
    if (
      ...
      if (userToRemoveIndex !== -1) {
        return res.status(400).json({
          message:
            'User hat nicht die Berechtigung, einen Moderator zu entfernen!',
        });
      }
    }
     ...
     */
    drawingAreas[drawingAreaIndex].authorizedUsers = drawingAreas[
      drawingAreaIndex
    ].authorizedUsers.filter(
      (authorizedUser) => authorizedUser.userId !== userIdToRemove
    );
    writeDrawingAreasToFile(drawingAreas);
     ...
     */
    users[userIndex].drawingAreas = users[userIndex].drawingAreas.filter(
      (drawingArea) => drawingArea.id !== drawingAreaId
    );
...
export default drawingAreaRouter;
